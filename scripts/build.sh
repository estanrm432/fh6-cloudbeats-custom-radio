...
if [ -d "$root/media" ]; then
    cp -r "$root/media" "$dist/media"
    printf '\033[90m-> media overlay bundled from repo media/\033[0m\n'
else
    printf '\033[33m-> media/ not found in repo; run fetch-media.sh separately if needed.\033[0m\n'
fi
