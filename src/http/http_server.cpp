    ...
    return json{
        ...
        {"playback",
         json{
             ...
             {"show_track_notification", c.playback.show_track_notification},
         }},
    };
}
...
void apply_patch(Config& c, const json& j) {
    ...
    if (auto it = j.find("playback"); it != j.end()) {
        ...
        c.playback.show_track_notification =
            pull(*it, "show_track_notification", c.playback.show_track_notification);
...
struct HttpServer::Impl {
    ...
    void run(uint16_t port) {
        ...
        // Winsock sockets are inheritable by default. The audio sources spawn
        // helper processes (yt-dlp / ffmpeg / deno) with bInheritHandles=TRUE,
        // which would duplicate this listening socket into them -- a lingering
        // helper then keeps port 8420 open after the game exits. Mark it
        // non-inheritable so the port is released the instant FH6 closes.
        SetHandleInformation(reinterpret_cast<HANDLE>(srv_sock), HANDLE_FLAG_INHERIT, 0);
        // SO_EXCLUSIVEADDRUSE (not SO_REUSEADDR): on Windows, SO_REUSEADDR lets
        // a second socket bind the SAME port. That let the desktop app's
        // WebView2 stack end up sharing port 8420 with this server, so inbound
        // connections split between the two and the dashboard answered only
        // intermittently. Exclusive use guarantees this server is the one and
        // only owner of the port.
        BOOL yes = TRUE;
        setsockopt(srv_sock, SOL_SOCKET, SO_EXCLUSIVEADDRUSE,
        ...
        // Bind the configured port and *only* that port. The dashboard, the
        // desktop app and any browser bookmark all target it, so silently
        // moving to a different port (the old port+1 fallback) just made the
        // site unreachable while the log claimed success. If the port is still
        // momentarily held by a previous instance that's shutting down, retry
        // briefly; otherwise fail loudly rather than hide on a phantom port.
        bool bound_ok = false;
        for (int attempt = 0; attempt < 20; ++attempt) {
            ...
            log::warn("[http] port {} busy (attempt {}/20) -- retrying in 250ms", port, attempt + 1);
            Sleep(250);
        }
        if (!bound_ok) {
            log::error("[http] could not bind port {} after retries -- another instance "
                       "or a stale process is still holding it", port);
        ...
        while (!stopping.load(std::memory_order_acquire)) {
            ...
            // Same inheritance guard for accepted connections (a request that
            // triggers a helper spawn must not leak the client socket either).
    ...
    void dispatch(SOCKET client, const Request& req, Ok&& ok, Fail&& fail) {
        ...
        if (m == "POST" && p == "/api/source/switch") {
            ...
            if (!mgr.switch_to(src)) return fail(404, "unknown source");
            // Remember the last active source so it is restored on next launch.
            store.patch([&src](Config& c) { c.general.default_source = src; });
            return ok();
        }
        if (m == "POST" && p == "/api/source/youtube_music/cast") {
            ...
            store.patch([](Config& c) { c.general.default_source = "youtube_music"; });
            return ok();
        }
        ...
        if (m == "POST" && p == "/api/source/jellyfin/cast") {
            ...
            store.patch([](Config& c) { c.general.default_source = "jellyfin"; });
            return ok();
        }
        if (m == "POST" && p == "/api/source/spotify/cast") {
            auto* sp = find_typed<sources::SpotifySource>("spotify");
            if (!sp) return fail(404, "spotify not registered");
            auto url = json::parse(req.body).value("url", std::string{});
            if (url.empty()) return fail(400, "url required");
            const bool was_active = (mgr.active() == sp);
            if (!sp->cast(url)) return fail(502, "could not resolve Spotify link (private/empty?)");
            if (was_active) mgr.ring().drain();
            mgr.switch_to("spotify");
            store.patch([&url](Config& c) {
                c.general.default_source = "spotify";
                c.spotify.default_playlist = url;
            });
            return ok();
        }
        if (m == "POST" && p == "/api/source/spotify/shuffle") {
            auto* sp = find_typed<sources::SpotifySource>("spotify");
            if (!sp) return fail(404, "spotify not registered");
            auto shuffle = json::parse(req.body).at("shuffle").get<bool>();
