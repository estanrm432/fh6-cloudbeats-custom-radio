...
// Bump a YouTube thumbnail URL to the highest practical resolution.
//  - YouTube Music cover art is served from googleusercontent.com with a size
//    suffix like "=w60-h60-l90-rj"; we swap it for a large square crop.
//  - Plain video thumbnails (i.ytimg.com/vi/<id>/<name>.jpg) get pinned to
//    maxresdefault when a lower-res variant was returned.
std::string upscale_thumbnail_url(std::string s) {
    // lh3/yt3.googleusercontent.com style: strip the trailing "=..." params and
    // request a 544x544 crop. "-rj" forces JPEG, "-l90" sets quality 90.
    if (s.find("googleusercontent.com") != std::string::npos ||
        s.find("ggpht.com") != std::string::npos) {
        if (auto eq = s.rfind('='); eq != std::string::npos)
            s.erase(eq);
        s += "=w544-h544-l90-rj";
        return s;
    }
    // i.ytimg.com video thumbnail: upgrade the filename to maxresdefault.
    if (s.find("ytimg.com") != std::string::npos) {
        for (const char* lo : {"/hqdefault", "/mqdefault", "/sddefault", "/default"}) {
            if (auto p = s.find(lo); p != std::string::npos) {
                s.replace(p, std::char_traits<char>::length(lo), "/maxresdefault");
                break;
            }
        }
    }
...
struct YouTubeMusicSource::Pipe {
    ...
    TrackInfo info{};
    // When true (Spotify), the title pipe only fills in artwork (the YouTube
    // thumbnail); title/artist stay as the Spotify-supplied metadata.
...
void YouTubeMusicSource::load_external_queue(std::vector<std::string> tokens,
                                            std::vector<TrackInfo> metas) {
    std::scoped_lock lk{mu_};
    discard_prefetch_locked();
    external_queue_  = true;
    queue_           = std::move(tokens);
    queue_meta_      = std::move(metas);
    queue_meta_.resize(queue_.size());   // keep arrays parallel
    queue_idx_       = 0;
    queue_built_for_ = "<external>";
    if (cfg_.shuffle && queue_.size() > 1) {
        // Shuffle both parallel arrays with the same permutation.
        static thread_local std::mt19937 rng{std::random_device{}()};
        for (std::size_t i = queue_.size() - 1; i > 0; --i) {
            std::uniform_int_distribution<std::size_t> d(0, i);
            std::size_t j = d(rng);
...
void YouTubeMusicSource::resolve_queue_locked() {
    if (external_queue_) return;   // queue was injected (Spotify); never resolve a URL
...
YouTubeMusicSource::spawn_pipe_locked(std::string_view url, std::size_t for_idx) {
    ...
    // External (Spotify) queue: we already know the real title/artist/artwork,
    // so seed them now and skip spawning the yt-dlp title resolver below.
    const bool have_meta = external_queue_ && for_idx < queue_meta_.size() &&
                           !queue_meta_[for_idx].title.empty();
    if (have_meta) {
        pipe->info        = queue_meta_[for_idx];
        pipe->meta_locked = true;   // keep Spotify title/artist; pull only YT artwork
    }
    ...
    // Always spawn the title resolver: for YouTube it fills title/artist/
    // duration/artwork; for Spotify (meta_locked) it contributes only the
    // YouTube thumbnail so the now-playing art box shows an image.
...
void YouTubeMusicSource::start_pipe_locked() {
    ...
    // Drain the title pipe synchronously so the very first track after
    // startup resolves its metadata before the control loop even ticks.
    // Without this, the title-resolver yt-dlp can finish before pump()
    // ever runs, leaving the buffered title/artist/artwork unread and the
    // dashboard stuck on "(loading)" forever.
    //
    // We wait up to ~3 s; if the resolver is still busy, pump() will
    // pick it up on the next tick.
    drain_title_pipe_blocking_locked(pipe_.get(), 3000);
}
...
void YouTubeMusicSource::play() {
    std::scoped_lock lk{mu_};
    if (!pipe_) {
        // Fresh start: start_pipe_locked() leaves state at `buffering`; pump()
        // promotes it to `playing` once real PCM is flowing. This keeps the
        // loading spinner visible during the yt-dlp/ffmpeg spin-up.
        start_pipe_locked();
        return;
    }
    // Resuming from pause.
    if (state_.load(std::memory_order_acquire) == PlaybackState::paused)
...
void YouTubeMusicSource::next() {
    ...
    auto i       = static_cast<std::ptrdiff_t>(queue_idx_) + 1;
    queue_idx_   = static_cast<std::size_t>(((i % n) + n) % n);
    // A promoted prefetch already has buffered PCM -> play immediately; a fresh
    // pipe stays `buffering` (spinner) until pump() sees real audio.
    if (promote_prefetch_locked(queue_idx_)) {
...
void YouTubeMusicSource::previous() {
    ...
    discard_prefetch_locked();
    start_pipe_locked();   // buffering; pump() promotes to playing when PCM flows
}
...
void YouTubeMusicSource::drain_title_pipe_blocking_locked(Pipe* p, unsigned timeout_ms) {
    if (!p || !p->title_pipe) return;

    // Wait for the title resolver to produce at least one byte, or exit.
    auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds{timeout_ms};
    while (std::chrono::steady_clock::now() < deadline) {
        DWORD tavail = 0;
        if (!PeekNamedPipe(p->title_pipe, nullptr, 0, nullptr, &tavail, nullptr)) break;
        if (tavail > 0) break;
    ...
    // Now drain whatever is available (or process the completed output).
...
void YouTubeMusicSource::drain_title_pipe_locked(Pipe* p) {
    ...
    auto thumbnail = take_line();
    // For Spotify (meta_locked) keep the Spotify title/artist; only adopt the
    // YouTube thumbnail (and duration if Spotify didn't provide one).
    if (!p->meta_locked) {
    ...
    try {
        if (!duration.empty() && duration != "NA" &&
            (!p->meta_locked || p->info.duration_ms == 0))
            p->info.duration_ms = static_cast<std::uint64_t>(std::stod(duration) * 1000.0);
    } catch (...) {}
    if (!thumbnail.empty() && thumbnail != "NA")
        p->info.artwork_url = upscale_thumbnail_url(thumbnail);
...
void YouTubeMusicSource::pump(RingBuffer& ring) {
    ...
    auto advance_to_next = [&] {
        ...
        if (promote_prefetch_locked(queue_idx_)) {
            ...
            start_pipe_locked();   // buffering until PCM flows
        }
    };
