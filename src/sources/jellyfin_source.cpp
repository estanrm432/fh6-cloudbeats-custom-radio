...
std::optional<std::string> http_get(const JellyfinConfig& cfg, const std::string& path) {
    ...
    WinHttpHandle session{WinHttpOpen(L"Horizon CloudBeats/1.0",
...
std::optional<std::vector<JellyfinTrack>> fetch_tracks(const JellyfinConfig& cfg) {
    ...
    try {
        ...
        for (const auto& item : *items) {
            ...
            // Primary image tag → used to build the artwork URL. Fall back to
            // the album's primary image when the track itself has none.
            if (auto tags = item.find("ImageTags");
                tags != item.end() && tags->is_object()) {
                if (auto pr = tags->find("Primary"); pr != tags->end() && pr->is_string())
                    t.image_tag = pr->get<std::string>();
            }
...
// Build a browser-loadable Primary image URL for a track. Jellyfin serves
// images on a public endpoint; the api_key is appended so locked-down servers
// still resolve it. fillWidth/Height request a high-quality square crop.
std::string artwork_url_for(const JellyfinConfig& cfg, const JellyfinTrack& t) {
    if (t.id.empty() || t.image_tag.empty() || cfg.server_url.empty()) return {};
    std::string base = cfg.server_url;
    while (!base.empty() && base.back() == '/') base.pop_back();
    return std::format(
        "{}/Items/{}/Images/Primary?tag={}&fillWidth=600&fillHeight=600&quality=90&api_key={}",
        base, t.id, t.image_tag, cfg.api_key);
}
...
TrackInfo JellyfinSource::current_track() const {
    ...
    info.album       = t.album;
    info.artwork_url = artwork_url_for(cfg_, t);
