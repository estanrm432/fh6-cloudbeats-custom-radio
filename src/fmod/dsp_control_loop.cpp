...
void ControlLoop::push_metadata() noexcept {
    ...
    if (!a) {
        meta_.update("Horizon CloudBeats", "Idle");
        return;
    }
    TrackInfo info;
    try {
    ...
    const bool show_notif = opts && opts->show_track_notification;

    if (show_notif) {
        // Song title written to DisplayName: game treats it as a station name
        // change and shows the notification banner on every track.
        ...
        // Keep DisplayName fixed so the game never sees a "station change" and
        // the notification banner stays silent between tracks. Song info is
        // combined into the subtitle (artist) slot instead.
        std::string combined;
        ...
        if (!title.empty() && !artist.empty())
            ...
            switch (a->playback_state()) {
                case PlaybackState::playing:   combined = "Playing"; break;
                case PlaybackState::buffering: combined = "Buffering"; break;
                case PlaybackState::paused:    combined = "Paused"; break;
                case PlaybackState::stopped:   combined = "Stopped"; break;
            }
        }
        meta_.update("Horizon CloudBeats", combined);
    }
}
