...
void run_bridge(HMODULE self) noexcept {
    ...
    log::info("[bridge] Horizon CloudBeats starting; data_dir={}", data_dir.string());
    ...
    auto sync_sources = [&mgr](const Config& c) {
        ...
        if (c.spotify.enabled && !mgr.find("spotify")) {
            // Spotify streams its matched audio through YouTube, so the inner
            // engine reuses the user's YouTube settings (yt-dlp path, cookies).
            auto src = std::make_unique<sources::SpotifySource>(c.spotify, c.youtube_music,
                                                                c.general.ffmpeg_path);
            if (src->initialize()) mgr.register_source(std::move(src));
